chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'saveMindMap') {
        ...
        chrome.storage.local.set({ 'mind-map-data': request.data }, () => {
            // Send data to server
            ...
            .then(data => {
                ...
                sendResponse({ success: true, message: 'Course data saved and sent to server' });
            })
            .catch(error => {
                console.error('Error sending course data to server:', error);
                sendResponse({ success: true, message: 'Course data saved locally but failed to send to server' });
            });
        });
        ...
        chrome.storage.local.get('mind-map-data', (result) => {
            if (result['mind-map-data']) {
                ...
                const blob = new Blob([JSON.stringify(result['mind-map-data'], null, 2)], { type: 'application/json' });
