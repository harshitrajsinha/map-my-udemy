document.addEventListener('DOMContentLoaded', function() {
    ...
    async function getCourseInfo() {
        try {
            ...
            const curriculumElement = document.querySelector('[data-purpose="course-curriculum"]');
            if (!curriculumElement) {
                return null;
            }

            // Ensure curriculum is expanded
            const expandButton = curriculumElement.querySelector('[data-purpose="expand-toggle"]');
            ...
            // Get instructor name
            const instructorElement = document.querySelector('.ud-instructor-links span');
            const instructorName = instructorElement ? instructorElement.textContent.trim() : null;

            // Get course sections and their items
            const sections = [];
            const panelContainer = curriculumElement.querySelector('div:not([class]):not([id]):not([name]):not([style])');
            if (!panelContainer) {
                return null;
            }

            // Find all accordion panels
            const panels = panelContainer.querySelectorAll('.accordion-panel-module--panel--Eb0it.section--panel--qYPjj');
            
            // Process each panel
            panels.forEach(panel => {
                // Get section title
                const sectionTitle = panel.querySelector('.ud-accordion-panel-heading .section--section-title--svpHP');
    ...
    checkButton.addEventListener('click', async function() {
        ...
        try {
            ...
            if (parsedUrl.hostname !== 'www.udemy.com' || 
            ...
            showResult('Error checking course. Please make sure you are on a Udemy course page.', 'error');
        }
    });
